...
void bt_init(bt_config_t *config, int argc, char **argv) {
    ...
    strcpy(config->output_file, "output.dat"); // 将相应文件名赋予bt config
    ...
    while ((c = getopt(argc, argv, _bt_optstring)) != -1) {
        switch (c) {
            ...
            case 'd':
                ...
                break;
            case 'p':
                strcpy(config->peer_list_file, optarg); // optarg 对应的是相应参数 -p <xxxx> 中的 <xxxx>
                break;
            case 'c':
