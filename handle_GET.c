...
up_conn_t *create_up_conn(bt_peer_t *peer, data_packet_t **pkts) { // 用数组控制在发送数据时更为方便
    assert(pkts != NULL);
...
up_conn_t *add_to_up_pool(up_pool_t *pool, bt_peer_t *peer, data_packet_t **pkts) {
    up_conn_t *conn = create_up_conn(peer, pkts);
    for (int i = 0; i < pool->max_conn; i++) {
...
down_conn_t *add_to_down_pool(down_pool_t *pool, bt_peer_t *peer, chunk_t *chunk) {
    down_conn_t *conn = create_down_conn(peer, chunk);
    for (int i = 0; i < pool->max_conn; i++) {
        if (pool->conns[i] == NULL) {
            pool->conns[i] = conn;
            break;
...
//chunk函数
chunk_t *make_chunk(int id, const uint8_t *sha_1) {
    chunk_t *chunk = malloc(sizeof(chunk_t));
    chunk->num = 0;
    chunk->flag = 0;
    chunk->inuse = 0;
    chunk->id = id;
    memcpy(chunk->sha_1, sha_1, SHA1_HASH_SIZE);
    chunk->data = malloc(BT_CHUNK_SIZE);
    chunk->peers = create_queue();
    return chunk;
}
...
task_t *init_task(const char *output_file, const char *chunk_file, int max_conn) {
    ...
    while (fgets(read_buf, 64, fp) != NULL) {
        ...
        hex2binary(sha_buf, 2 * SHA1_HASH_SIZE, sha_1);
        chunk = make_chunk(i, sha_1);
...
void available_peer(task_t *task, uint8_t *sha_1, bt_peer_t *peer) {
    ...
    for (node *current = task->chunks->head; current != NULL; current = current->next) {
        ...
        if (memcmp(sha_1, chunk->sha_1, SHA1_HASH_SIZE) == 0) {
            if (chunk->peers == NULL) {
                chunk->peers = create_queue();
            }
...
char *find_chunk_data(task_t *task, uint8_t *sha_1) {
    ...
    for (node *cur = task->chunks->head; cur != NULL; cur = cur->next) {
        chunk = cur->data;
        if (memcmp(chunk->sha_1, sha_1, SHA1_HASH_SIZE) == 0) {
            return chunk->data;
        }
    }
    return NULL;
...
int check_task(task_t *task) {
    ...
    for (node *cur = task->chunks->head; cur != NULL; cur = cur->next) {
        chunk = cur->data;
        shahash((uint8_t *) chunk->data, BT_CHUNK_SIZE, sha_1);
        if (memcmp(sha_1, chunk->sha_1, SHA1_HASH_SIZE) != 0) {
            flag = 0;
            chunk->flag = 0;
            chunk->inuse = 0;
...
chunk_t *choose_chunk(task_t *task, queue *chunks, bt_peer_t *peer) {
    ...
    chunk_t *chunk;
    chunk_t *ret = NULL;
    while ((sha_1 = dequeue(chunks)) != NULL) {
        for (node *cur = task->chunks->head; cur != NULL; cur = cur->next) {
            chunk = cur->data;
            if (!chunk->inuse && !chunk->flag && memcmp(sha_1, chunk->sha_1, SHA1_HASH_SIZE) == 0) {
                if (ret == NULL) {
                    ret = chunk;
                }
                enqueue(chunk->peers, peer);
...
void continue_task(task_t *task, down_pool_t *pool, int sock) {
    ...
    if (flag) {
        ...
        chunk->inuse = 1;
        data_packet_t *pkt = create_get_packet(SHA1_HASH_SIZE + HEADERLEN, (char *) chunk->sha_1);
        send_packet(sock, pkt, (struct sockaddr *) &peer->addr);
        free_packet(pkt);
    }
    if (q->n) {
        queue *ret = init_chunk_packet_queue(q, create_whohas_packet);
